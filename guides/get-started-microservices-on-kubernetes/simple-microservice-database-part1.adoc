...
We need a database, and we will use PostgreSQL in its containerized version: see link:https://hub.docker.com/_/postgres[PostgreSQL Official Image];
...
We will extend the sample application we created in link:simple-microservice-part1[{simple-microservice-part1}];

To connect to any database, we need to:

* install the JDBC Drivers for the PostgreSQL Database into WildFly
* create a database connection in WildFly

To achieve both, we will use the link:https://github.com/wildfly-extras/wildfly-datasources-galleon-pack[wildfly-datasources-galleon-pack]: this is a feature-packs for WildFly and, as we know, feature-packs are the building blocks of WildFly and can be used to add capabilities to WildFly;

=== pom.xml

==== dependencies

Add the following dependencies to the `pom-xml` file `dependencies`:
