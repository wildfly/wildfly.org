...
In this post, we will extend the example created in link:simple-microservice-part1[{simple-microservice-part1}] and add Database connectivity;
...
We will use PostgreSQL in this post in its containerized version: see link:https://hub.docker.com/_/postgres[PostgreSQL Official Image];
...
podman run --rm --network={containers-demo-network} --name {postgre-sql-pod-name} \
...
NOTE: We created the *{containers-demo-network}* network and started the *{postgre-sql-pod-name}* container with the `--network={containers-demo-network}` option: later in this guide, this will allow us to connect to the *{postgre-sql-pod-name}* container from the *{my-jaxrs-app-db-docker-image-name}* container;

== Maven Project

You will extend the sample application you created in link:simple-microservice-part1[{simple-microservice-part1}] by adding the link:https://github.com/wildfly-extras/wildfly-datasources-galleon-pack[wildfly-datasources-galleon-pack];

The link:https://github.com/wildfly-extras/wildfly-datasources-galleon-pack[wildfly-datasources-galleon-pack] will:
...
.org.wildfly.examples.test_table.model.TestTable   :
[source,java]
----
...
.org.wildfly.examples.test_table.service.TestTableResource :
[source,java]
----
...
Note that, before starting the application, we specify some *environment variables* used by WildFly to connect to the PostgreSQL database:

[source,bash,subs="normal"]
----
export POSTGRESQL_USER={postgre-sql-user}
export POSTGRESQL_PASSWORD={postgre-sql-password}
export POSTGRESQL_HOST={postgre-sql-host}
export POSTGRESQL_PORT={postgre-sql-port}
export POSTGRESQL_DATABASE={postgre-sql-database}
export POSTGRESQL_JNDI={postgre-sql-jndi}

./target/server/bin/standalone.sh
...
16:26:11,052 INFO  [org.jboss.as] (Controller Boot Thread) WFLYSRV0025: WildFly Full 31.0.0.Final (WildFly Core 23.0.1.Final) started in 2840ms - Started 310 of 389 services (150 services are lazy, passive or on-demand) - Server configuration file in use: standalone.xml
----
...
NOTE: We started the *{my-jaxrs-app-db-docker-image-name}* container with the `--network={containers-demo-network}` option just like we did when we started the *{postgre-sql-pod-name}* container: the two containers now run in the same *{containers-demo-network}* network and we can connect to the *{postgre-sql-pod-name}* container from the *{my-jaxrs-app-db-docker-image-name}* container using the *{postgre-sql-pod-name}* DNS name;

=== Check the application

Repeat the checks in <<check_the_application>>;

NOTE: you might find your data twice if didn't restart the Database
...
* Source code for this post: {source-code-git-repository}/tree/simple-microservice-database-part1
