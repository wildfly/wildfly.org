...
In this guide, you will learn HOW-TO run the Docker Image we built in link:simple-microservice-llm-part1[{simple-microservice-llm-part1}] on Kubernetes.
...
* Complete link:simple-microservice-llm-part1[{simple-microservice-llm-part1}]

include::_includes/_constants.adoc[]

== LLM

We basically will repeat everything we did in link:simple-microservice-part2[{simple-microservice-part2}] with a few changes but, before that, we will deploy link:https://ollama.com/library/smollm2[`{ollama-llm}`, window="_blank"] on Kubernetes using the Ollama container.
