...
In this post, you will learn HOW-TO run the Docker Image we built in link:simple-microservice-database-part1[{simple-microservice-database-part1}] on Kubernetes.
...
NOTE: if you are using link:https://minikube.sigs.k8s.io/docs/[minikube], and you want to test if PostgreSQL is working, run `kubectl port-forward svc/{postgre-sql-kubernetes-service-name} {postgre-sql-port}:{postgre-sql-port}` and then connect via JDBC using URL `jdbc:postgresql://{postgre-sql-host}:{postgre-sql-port}/{postgre-sql-database}`
...
There are many options to achieve this; in this post, we will push the `{my-jaxrs-app-db-docker-image-name}:latest` Docker Image, to the link:quay.io[quay.io] Image Registry;

NOTE: You can skip this step in case you prefer to re-use the `quay.io/{quay-io-account-name}/{my-jaxrs-app-db-docker-image-name}` Docker Image, that we already pushed to link:quay.io[quay.io] in the `{quay-io-account-name}` account;

Create a public repository named `{my-jaxrs-app-db-docker-image-name}` on link:quay.io[quay.io] (e.g. link:https://quay.io/repository/{quay-io-account-name}/my-jaxrs-app-db[https://quay.io/repository/{quay-io-account-name}/my-jaxrs-app-db]);
...
To deploy our `{my-jaxrs-app-db-docker-image-name}` Docker Image on link:https://minikube.sigs.k8s.io/docs/[minikube], create a file named `deployment-{my-jaxrs-app-db-docker-image-name}.yaml` (see link:https://kubernetes.io/docs/concepts/workloads/controllers/deployment/[kubernetes deployment]) in the same directory as the `Dockerfile` and the `pom.xml` file, with the following content:
...
* Source code for this post: {source-code-git-repository}/tree/simple-microservice-database-part2
