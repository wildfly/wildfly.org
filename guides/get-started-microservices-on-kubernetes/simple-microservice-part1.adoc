...
In this post, you will learn how to create and run a link:https://docs.docker.com/guides/docker-concepts/the-basics/what-is-an-image/[Docker Image] containing a link:https://jakarta.ee/specifications/restful-ws/[Jakarta RESTful Web Service] implemented using WildFly.

include::{includedir}/_prerequisites.adoc[]

* Complete link:/get-started[Getting Started with WildFly]

include::_includes/_constants.adoc[]

== Maven Project

In link:/get-started[Getting Started with WildFly], we created a simple Jakarta EE application Maven Project containing a link:https://jakarta.ee/specifications/restful-ws/[Jakarta RESTful Web Service] using an existing archetype:
...
We need to add some features that are necessary when, in the next post, we will run our application on link:https://kubernetes.io/[Kubernetes]; specifically, we need to expose link:https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/[Liveness, Readiness and Startup Probes];

We can add these features by adding the link:https://github.com/wildfly-extras/wildfly-cloud-galleon-pack[wildfly-cloud-galleon-pack] to our application; add the following to the the link:https://github.com/wildfly/wildfly-maven-plugin/[wildfly-maven-plugin] configuration in the `pom.xml` file:
...
* link:http://127.0.0.1:8080/hello/pippo[http://127.0.0.1:8080/hello/pippo]
...
We will use the link:https://docs.docker.com/build/guide/intro/#the-dockerfile/[Dockerfile] from link:https://github.com/wildfly/wildfly-s2i/blob/main/examples/docker-build/Dockerfile[examples/docker-build/Dockerfile] because it is crafted specifically for WildFly;
...
NOTE: As you can see, we used link:https://podman.io/[Podman], but you can also use link:https://www.docker.com/[Docker], depending on your preference and on which one you have installed on your PC; to switch to Docker, replace *podman* with *docker* in the previous command (and in the ones that will follow);
...
Just like we did before, go to this URL:
...
Same as before, go to these URLs:
