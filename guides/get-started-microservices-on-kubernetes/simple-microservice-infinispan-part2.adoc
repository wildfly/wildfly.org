...
Create e file named `infinispan-configmap.yaml` with the following content:
...
Create e file named `infinispan-deployment.yaml` with the following content:
...
Create e file named `infinispan-service.yaml` with the following content:
...
There are many options to achieve this; in this post, we will push the `{my-jaxrs-app-infinispan-docker-image-name}:latest` Docker Image, to the link:https://quay.io[quay.io, window="_blank"] Image Registry;

NOTE: You can skip this step in case you prefer to re-use the `quay.io/{quay-io-account-name}/{my-jaxrs-app-infinispan-docker-image-name}` Docker Image, that we already pushed to link:https://quay.io[quay.io, window="_blank"] in the `{quay-io-account-name}` account;
