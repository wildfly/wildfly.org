...
We will use Apache Artemis in this post in its containerized version: see link:https://quay.io/artemiscloud/activemq-artemis-broker-kubernetes[Artemis Official Image, window="_blank"];
...
[source,bash]
----
$ curl -X GET http://localhost:8080/api/message/send?content=Hello%20World
...
* Source code for this post: {source-code-git-repository}/tree/simple-microservice-jms-part1
