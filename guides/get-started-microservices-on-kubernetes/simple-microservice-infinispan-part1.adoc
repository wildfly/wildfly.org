...
----
podman network create {containers-demo-network}

podman run --rm --network={containers-demo-network} --name={infinispan-pod-name} \
...
NOTE: NOTE: We created the *{containers-demo-network}* network and started the *{infinispan-pod-name}* container with the `--network={containers-demo-network}` option: later in this guide, this will allow us to connect to the *{infinispan-pod-name}* container from the *{my-jaxrs-app-infinispan-docker-image-name}* container;

NOTE: you can check the container is running by opening http://0.0.0.0:{infinispan-port}/console/ in your browser and logging in as `{infinispan-user}` / `{infinispan-password}`
...
[source,bash,subs="normal"]
----
podman run --rm --network={containers-demo-network} -p 8080:8080 -p 9990:9990 \
...
NOTE: NOTE: We started the *{my-jaxrs-app-infinispan-docker-image-name}* container with the `--network={containers-demo-network}` option just like we did when we started the *{infinispan-pod-name}* container: the two containers now run in the same *{containers-demo-network}* network and we can connect to the *{infinispan-pod-name}* container from the *{my-jaxrs-app-infinispan-docker-image-name}* container using the *{infinispan-pod-name}* DNS name;
