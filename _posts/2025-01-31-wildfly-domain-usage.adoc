...
I won’t explain much about the concept of WildFly’s domain mode. I have written a book before, explaining the domain mode usage of WildFlyfootnote:[https://www.packtpub.com/en-us/product/jboss-eap6-high-availability-9781783282449?srsltid=AfmBOooUR6olUaY8qVHB6U70wb2oe_nAk8OloSeJAtkbn6AGgoQ7-OM9[JBoss EAP6 High Availability | Programming | eBook]]. There is also a relative document about the domain mode setupfootnote:[https://docs.wildfly.org/35/High_Availability_Guide.html[High Availability Guide]]. Nevertheless, the resources I could find are mostly outdated or don't contain the step-by-step walkthrough about the minimal configuration to be done for using the domain mode of WildFly. Especially in the security configuration part related to Elytron, according to the blog post written by Brian Stansberryfootnote:[https://www.wildfly.org/news/2021/10/05/WildFly25-Final-Released/[WildFly
25 is released!]]:
...
Please note that we didn’t generate a secret value related to the generated `admin` user, which is already deprecated. So this kind of configuration on the Secondary Host Controller is deprecated:
...
As the Primary Host Controller is started on Machine A, now we can check the WildFly server configuration on Machine B. As planned, the WildFly server is named `secondary`, and the server will be connected to the Primary Host Controller on Machine A, which acts as the domain controller.

In the Secondary Host Controller on Machine B, it will use the `host-secondary.xml` as its server configuration. We need to make some modifications to this file. First I need to add the `name` property to the `host` section:
...
As the configuration is shown above, I added a `remote` configuration into the `domain-controller` section, and defined the `authentication-context` to be used as `myCtx`. This configuration is different from the legacy authentication. So I need to define this `authentication-context` in the `elytron` subsystem. Here are the details:
...
From the above Primary Host Controller log output, I can see the secondary host is connected, so it starts to accept the management of Primary Host Controller, which acts as the domain controller. Now I can open the web browser on Machine A to access the WildFly server admin console:
