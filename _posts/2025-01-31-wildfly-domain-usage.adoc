...
You are disconnected at the moment. Type 'connect' to connect to the server or 'help' for the list of supported commands.
[disconnected /] embed-host-controller --host-config=host-secondary.xml
[domain@embedded /] /host=localhost:write-attribute(name=name,value=secondary)
{
"outcome" => "success",
"result" => undefined,
"server-groups" => undefined,
...
[domain@embedded /]
[domain@embedded /] /host=secondary:reload(admin-only=true)
{"outcome" => "success"}
----

Please note that if you use tab complete, it should autocomplete your default host name. Giving the host a name can help to analyze the server log output later. Then I need to add another configuration to the `domain-controller`:
...
[source,bash]
----
/host=secondary/subsystem=elytron/authentication-configuration=myConfig:add(sasl-mechanism-selector=DIGEST-MD5, authentication-name=admin, realm=ManagementRealm)
/host=secondary/subsystem=elytron/authentication-context=myCtx:add(match-rules=[{match-host="${jboss.domain.primary.address}", authentication-configuration=myConfig}])
----
