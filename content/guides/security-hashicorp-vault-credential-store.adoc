...
Download the Galleon provisioning tools and use them to provision a WildFly server with the HashiCorp Vault subsystem. Run the following command, substituting the version with the latest version in link:https://mvnrepository.com/artifact/org.wildfly.security.hashicorp.vault/wildfly-hashicorp-vault-feature-pack[maven central]:

[source,bash]
----
galleon.sh install org.wildfly:wildfly-hashicorp-vault-feature-pack:${version} \
...
=== Step 2: Start WildFly with Experimental Stability
...
We are working on adding expression resolver functionality which will be backed by HashiCorp Vault.
...
=== You can build the feature pack directly from the repository
...
And you can use the SNAPSHOT versions as usual:

[source,bash]
----
galleon.sh install org.wildfly:wildfly-hashicorp-vault-feature-pack:${version-SNAPSHOT} \
--layers=hashicorp-vault \
--dir=wildfly
----
