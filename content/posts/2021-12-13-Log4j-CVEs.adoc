...
To use this, update the `$JBOSS_HOME/bin/standalone.conf` or `$JBOSS_HOME/bin/domain.conf` file (or, for Windows users, the `.bat` or `.ps1` variants of those) to add `-Dlog4j2.formatMsgNoLookups=true` to the value of the `JAVA_OPTS` environment variable. (Which file you update depends on whether you are running a standalone server or a domain mode host.)
